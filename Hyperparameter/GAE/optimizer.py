...
class OptimizerAE(object):
    ...
    def __init__(self, preds, labels, degree_matrix, num_nodes, pos_weight, norm, clusters_distance,simple,fastgae,learning_rate,beta):
        ...
        if simple or fastgae: # simpler proxy of the modularity
        ...
        self.cost = self.cost_adj - beta * self.cost_mod
...
class OptimizerVAE(object):
    ...
    def __init__(self, preds, labels, degree_matrix, model, num_nodes, pos_weight, norm, clusters_distance,simple,fastgae,learning_rate,beta):
        ...
        if simple or fastgae: # simpler proxy of the modularity
            ...
            self.cost_mod = (1.0 / num_nodes) * tf.reduce_sum((labels_sub - degree_matrix) * clusters_distance)
        # Note: here, self.cost_adj corresponds to -ELBO. By minimizing (-ELBO-beta*self.cost_mod),
        # we actually maximize (ELBO+beta*self.cost_mod) as in the paper
        self.cost = self.cost_adj - beta * self.cost_mod
