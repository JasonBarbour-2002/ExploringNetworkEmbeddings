...
# For mu < 0.5
evaluate = evaluate_schulze(allvs2[:25] + allvs[:5*KLresults.shape[1]],10).candidate_wins
print(f'mu < 0.5')
for i in evaluate:
    str = ' '.join(Algos[i])
    print(str,' & ',sep='', end = '')
...
# For mu >= 0.5
evaluate = evaluate_schulze(allvs[5*KLresults.shape[1]:] + allvs2[25:],10).candidate_wins
print(f'mu >= 0.5')
for i in evaluate:
    str = ' '.join(Algos[i])
    print(str,end= '')
...
# For all mu
evaluate = evaluate_schulze(allvs+allvs2,10).candidate_wins
print(f'All mu')
for i in evaluate:
    str = ' '.join(Algos[i])
    print(str,end='')
