...
def metrics():
    if not os.path.exists('OldMetrics'):
        os.makedirs('OldMetrics')
    if not os.path.exists(f'OldMetrics/{L}'):
        os.makedirs(f'OldMetrics/{L}')
    ...
    np.save(f'OldMetrics/{L}/ami.npy',ami)
    np.save(f'OldMetrics/{L}/nmi.npy',nmi)
    np.save(f'OldMetrics/{L}/ari.npy',ari)
    np.save(f'OldMetrics/{L}/macro_f1.npy',macro_f1)
    np.save(f'OldMetrics/{L}/micro_f1.npy',micro_f1)


def analise():
    ami = np.load(f'OldMetrics/{L}/ami.npy')
    nmi = np.load(f'OldMetrics/{L}/nmi.npy')
    ari = np.load(f'OldMetrics/{L}/ari.npy')
    macro_f1 = np.load(f'OldMetrics/{L}/macro_f1.npy')
    micro_f1 = np.load(f'OldMetrics/{L}/micro_f1.npy')
    ...
    for idx,i in enumerate(list):
        ...
        plt.tight_layout()
        plt.savefig(f'OldMetrics/{L}/{Scores[idx]}.png',bbox_inches='tight')
        plt.clf()
