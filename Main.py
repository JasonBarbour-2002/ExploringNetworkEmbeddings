...
def run(Algo,params,Graph,num_comu,L,i,Thread_id):
    ...
    F = open(f'Label_{L}/{i}/{Algo.__name__}/Times.txt','a')
    F.write(f'{t}\n')
    F.close()
...
def get_Embeddings(path,Algo,params,Thread_id):
    ...
    for i in R:
        ...
        if not os.path.exists(f'Label_{L}/{i}/{Algo.__name__}'):
...
if __name__ == '__main__':
    ...
    if algo == 'Deepwalk':
        from embed_utils import Deepwalk as Algo
    elif algo == 'n2v':
        from embed_utils import n2v as Algo
    elif algo == 'Walklet':
        from embed_utils import Walklet as Algo 
    elif algo == 'mnmf':
        from embed_utils import mnmf as Algo
    elif algo == 'boost':
        from embed_utils import boost as Algo
    elif algo == 'diff':
        from embed_utils import diff as Algo
    elif algo == 'LEM':
        from embed_utils import LEM as Algo
    elif algo == 'netmf':
        from embed_utils import netmf as Algo
    elif algo == 'randne':
        from embed_utils import randne as Algo
    elif algo == 'GAE':
        from embed_utils import GAE as Algo
    else:
    ...
    get_Embeddings('Graph_mu/',Algo,params,Thread_id)
